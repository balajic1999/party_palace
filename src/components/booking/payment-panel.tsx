...
export function PaymentPanel() {
  ...
  if (!draft || !q) {
    return (
      <div className="shell py-24 text-center">
        <h1 className="text-[28px] leading-tight">We lost track of that booking.</h1>
  ...
  return (
    <div className="shell py-10 md:py-16">
      <Link
        href="/book?step=5"
        className="group inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.14em] text-text-mid transition-colors hover:text-gold-700"
      >
        <ArrowLeft
          strokeWidth={1.5}
      ...
      <h1 className="mt-7 text-[clamp(2rem,4.8vw,2.9rem)] leading-[1.06]">
        One last look, then we hold it.
      </h1>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-12 lg:gap-10">
          ...
          <div className="rounded-md border border-line">
            <div className="flex items-start justify-between gap-4 border-b border-line p-6">
              <div>
                ...
                <h2 className="mt-2 text-[26px] leading-none text-text">
                  {pkg?.name}
                </h2>
              </div>
              <Link
            ...
            <dl className="grid gap-5 p-6 sm:grid-cols-2">
              <Fact icon={CalendarDays} k="Date" href="/book?step=2">
                {draft.date
              ...
              <Fact icon={Clock} k="Slot" href="/book?step=2">
                {slot ? `${to12h(slot.start)} – ${to12h(slot.end)}` : "—"}
              </Fact>
              <Fact icon={Users} k="Guests" href="/book?step=1">
                {draft.guests} guests
              </Fact>
            </dl>
                ...
                {extras.length > 0 && (
                  <div className={draft.screenMessage ? "mt-4" : ""}>
                    <div className="flex items-baseline justify-between gap-4">
                      ...
                      <Link
                        href="/book?step=4"
                        className="text-[12.5px] text-text-mid transition-colors hover:text-gold-700"
                      >
                        Change
                      </Link>
                    </div>
        ...
        <div className="lg:col-span-5">
          <div className="sticky top-[100px] rounded-md border border-line bg-white/60 p-6">
            <p className="text-[10.5px] uppercase tracking-[0.18em] text-gold-600">
              Pay the advance
            </p>

            <p className="mt-4 flex items-baseline gap-2">
              <span className="tnum text-[40px] leading-none text-text">
                {formatINR(q.advance)}
              </span>
